...
import { 
  ...
  Check, Clock, Eye, X, RefreshCw, Link, FileSignature, History, Copy, ClipboardCheck, Wifi
} from 'lucide-react';
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setAgreements(prevAgreements => {
        // Find candidates that can be updated (not Signed or Expired)
        const candidates = prevAgreements.filter(a => a.status === 'Sent' || a.status === 'Viewed');
        
        if (candidates.length === 0) return prevAgreements;

        // 40% chance to update a random record in this tick
        if (Math.random() > 0.6) {
            const candidate = candidates[Math.floor(Math.random() * candidates.length)];
            
            // Probability logic: Sent -> Viewed is likely, Viewed -> Signed is less likely
            const shouldUpdate = candidate.status === 'Sent' ? Math.random() > 0.3 : Math.random() > 0.6;
            
            if (!shouldUpdate) return prevAgreements;

            const now = new Date();
            const timestamp = now.toLocaleString('en-IN', { hour: '2-digit', minute: '2-digit', day: 'numeric', month: 'short' });

            const newAgreements = prevAgreements.map(a => {
              if (a.id !== candidate.id) return a;

              let updated = { ...a };
              if (updated.status === 'Sent') {
                updated.status = 'Viewed';
                updated.lastActivity = 'Viewed just now (Live)';
                updated.auditTrail = [...updated.auditTrail, { 
                  id: Math.random().toString(), 
                  timestamp, 
                  action: 'Viewed', 
                  details: 'Document opened by recipient via link' 
                }];
              } else if (updated.status === 'Viewed') {
                updated.status = 'Signed';
                updated.lastActivity = 'Signed just now (Live)';
                updated.auditTrail = [
                  ...updated.auditTrail, 
                  { id: Math.random().toString(), timestamp, action: 'Signed', details: `Digitally signed by ${updated.customerName}` },
                  { id: Math.random().toString(), timestamp, action: 'Sealed', details: 'Tamper-proof IP timestamp added' }
                ];
              }
              return updated;
            });
    ...
    setAgreements(prev => prev.map(a => {
      ...
      if (a.status === 'Sent') {
        return { 
          ...a, 
          status: 'Viewed', 
          lastActivity: 'Viewed just now',
          auditTrail: [
            ...a.auditTrail, 
            { id: Math.random().toString(), timestamp: timestamp, action: 'Viewed', details: 'Document opened by recipient' }
          ]
        };
      }
      ...
      if (a.status === 'Viewed') {
        return { 
          ...
          auditTrail: [
            ...a.auditTrail, 
            { id: Math.random().toString(), timestamp: timestamp, action: 'Signed', details: `Digitally signed by ${a.customerName}` },
            { id: Math.random().toString(), timestamp: timestamp, action: 'Sealed', details: 'Tamper-proof seal applied to PDF' }
          ]
        };
      }
      return a;
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 animate-scale-in">
                ...
                <div className="space-y-4">
                    ...
                    >
                        ...
                        {isSending ? 'Generate Secure Link & Send' : 'Send for Signature'}
                    </button>
                </div>
            </div>
         </div>
      ...
      <div className="flex justify-between items-center">
         <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 text-xs text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100 w-fit">
                <Wifi size={12} className="animate-pulse" /> Live Status Updates Active
            </div>
         </div>
         <button 
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-100">
            <tr>
              ...
              <th className="px-6 py-4">Unique Link</th>
            ...
            {agreements.map((agreement) => (
              <tr key={agreement.id} className="hover:bg-slate-50 transition-colors duration-300">
                <td className="px-6 py-4">
                   ...
                   <div className="text-xs text-slate-400">{agreement.recipientEmail}</div>
                </td>
                ...
                <td className="px-6 py-4">
                    <button 
                       onClick={() => copyLink(agreement.uniqueLink, agreement.id)}
                       className="flex items-center gap-2 text-xs bg-white border border-slate-200 px-2 py-1 rounded hover:bg-slate-50 text-slate-600 transition-colors"
                       title={agreement.uniqueLink}
                    >
                       <Link size={12} /> 
                       {copiedLinkId === agreement.id ? <span className="text-emerald-600 font-bold flex items-center gap-1">Copied <Check size={10}/></span> : 'Copy Link'}
                    </button>
                </td>
                <td className="px-6 py-4 text-slate-500 text-xs">{agreement.lastActivity}</td>
                <td className="px-6 py-4 text-center flex justify-center gap-2">
                   <button 
                     onClick={() => setViewAuditId(agreement.id)}
                   ...
                   <button 
                     ...
                     className="text-slate-500 hover:text-emerald-600 p-2 rounded-full hover:bg-emerald-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                     title="Manually Advance Status"
                   >
                      <RefreshCw size={16} />
                   </button>
                </td>
